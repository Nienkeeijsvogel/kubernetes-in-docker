...
# Create worker nodes
docker run -d --privileged --network kafka --name zookeeper1 icebert/kubernetes-in-docker
docker run -d --privileged --network kafka --name kafka-1 icebert/kubernetes-in-docker
